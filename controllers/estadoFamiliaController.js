...
var services = require("../services/estadoFamiliaServices");
const {
    manejarErrorRequest
} = require('../common/error-handler');

router.get('/', (req, res, next) => {
    services.obtener_estado_familia(req,req.query,"GET").then((response) => {
...
router.post('/', (req, res, next) => {
    services.insertar_estado_familia(req,req.body,"POST").then((response) => {
        if(response.status != CODE.OK){res.status(response.status).send(manejarErrorRequest(response));} 
        else {res.status(response.status).send(response);}
    }, (error) => {
        res.status(CODE.INTERNAL_SERVER_ERROR).send(manejarErrorRequest(error));        
...
router.put('/', (req, res, next) => {
    services.actualizar_estado_familia(req,req.body,"PUT").then((response) => {
        if(response.status != CODE.OK){res.status(response.status).send(manejarErrorRequest(response));} 
        else {res.status(response.status).send(response);}
    }, (error) => {
        res.status(CODE.INTERNAL_SERVER_ERROR).send(manejarErrorRequest(error));        
...
router.delete('/', (req, res, next) => {
    services.eliminar_estado_familia(req,req.query,"DELETE").then((response) => {
        if(response.status != CODE.OK){res.status(response.status).send(manejarErrorRequest(response));} 
        else {res.status(response.status).send(response);}
    }, (error) => {
        res.status(CODE.INTERNAL_SERVER_ERROR).send(manejarErrorRequest(error));        
