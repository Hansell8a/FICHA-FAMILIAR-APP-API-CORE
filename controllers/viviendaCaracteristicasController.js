...
var services = require("../services/viviendaCaracteristicaServices");
const {
    manejarErrorRequest
...
router.get('/', (req, res, next) => {
    services.obtener(req, req.query, "GET").then((response) => {
        if (response.status != CODE.OK) {
            res.status(response.status).send(manejarErrorRequest(response));
        } else {
...
router.post('/detalleUnificado', (req, res, next) => {
    console.log(req);
    services.insertarDetalleUnificado(req, req.body, "POST").then((response) => {
        if (response.status != CODE.OK) {
            res.status(response.status).send(manejarErrorRequest(response));
        } else {
            res.status(response.status).send(response);
